function [negloglike, nlls, pl, V_hist, omegaVs, RPE, Rel_diff] = funSideBias_dynamicVchoLinRelDec_2betaR(xpar,dat, initOmegaV)
%PURPOSE:   Function for maximum likelihood estimation, called by fit_fun().
% AUTHOR: Jae Hyung Woo, 11/03/2023
%INPUT ARGUMENTS
...
%% 
alpha       = xpar(1);
beta_DV     = xpar(2);        % max inv. temp for Stim
SideBias    = xpar(3);
alpha2      = xpar(4);
decay_rate  = xpar(5);
...
gammaUpdate = xpar(7);   % fit update rate for omegaV
omega0      = xpar(8);    % initial omegaV at trial 1
...
%% looping through trials
    ...
    DV_left = V_Stim.left*Rho*omega_vS + V.Left*(1-Rho)*omega_vL;
    DV_right = V_Stim.right*Rho*omega_vS + V.Right*(1-Rho)*omega_vL;
