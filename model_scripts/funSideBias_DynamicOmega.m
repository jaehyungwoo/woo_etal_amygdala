function [negloglike, nlls, pl, V_hist, omegas, RPE, Rel_diff] = funSideBias_DynamicOmega(xpar,dat, initOmegaV)
% % Dynamic omega model % 
% AUTHOR: Jae Hyung Woo, 08/02/2023
%PURPOSE:   Function for maximum likelihood estimation, called by fit_fun().
%
%INPUT ARGUMENTS
%   xpar:       model parameters (see below script for descriptions)
%       alpha   :   
%       beta_DV :   inverse temperature
%       alpha2  :   learning rate for unrewarded options
%   dat:        data
...
%   omegas    :      omega values by trial
%   RPE       :      RPE for each system (stimulus & action)
%   Rel_diff  :      Reliablity difference between systems, used to update omegaV

%% 
alpha       = xpar(1);      % learning rate for rewaraded options
beta_DV     = xpar(2);      % inverse temperature
...
gammaUpdate = xpar(6);      % baseline arbitration rate for updating omega
...
%% looping through trials
    ...
    omegas(k) = omegaV;    % store omegaV values for every trial
    omega_vS = omegaV;
    omega_vL = 1 - omegaV;
    ...
    if deltaRel>0
       omegaV = omegaV + gammaUpdate*deltaRel*(1-omegaV); 
    else
       omegaV = omegaV + gammaUpdate*abs(deltaRel)*(0-omegaV);
    end 
